...
--------------------------------
  :speaking_head:  **OBS: Se observar os 2 (dois) arquivos estão com as Middlewares de página de erros personalizada sendo roteados pelo serviço chamado "manutenção". Também a ipWhiteList impedindo a rede fictícia "10.10.0.0/22" de acessar determinado serviço conforme a necessidade. Por último foi criado o redirecionamento automático para https através da middleware "redirectscheme", tomei a iniciativa de deixar a configuração de ingressroute para fazer gerência dessa função, sendo assim elas estão junto no final dos arquivos middlewares.yaml** :shipit:
